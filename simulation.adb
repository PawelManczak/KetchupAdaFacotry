...
procedure Simulation is
   ...
   Product_Name: constant array (Product_Type) of String(1 .. 11)
     := ("pomidory   ", "pieprz     ", "sol        ", "cukier     ", "konserwanty");

  Assembly_Name: constant array (Assembly_Type) of String(1 .. 15)
     := ("Ketchup_Lagodny", "Ketchup_Ostry  ", "Ketchup_Natural");
   ...
   task body Producer is
      subtype Production_Time_Range is Integer range 1 .. 4;
   ...
   begin
	 ...
	 --Put_Line("Podprodukt:  Produced product " & Product_Name(Product_Type_Number)
    ...
    select
	 B.Take(Product_Type_Number, Product_Number);
            ...
            Put_Line("Ekipa magzazynowa jest zajeta, produkt trafia do inej firmy");
            delay Duration(13);
            end select;
      end loop;
   end Producer;
   ...
   begin
	 ...
	 if Assembly_Number = 0 then
        Put_line("Nie udalo sie zrealizowac zamowienia, sproboj ponownie pozniej!");
            else
             Put_Line(Consumer_Name(Consumer_Nb) & ": przyjal zestaw " &
   ...
   task body Buffer is
     ---
      ...
      Storage_Capacity_Of_Product: My_Int_Array := (10, 6, 6, 4, 4); --razen stirage capacity
	...
	:= ((1, 1, 0, 1, 2),
     ...
     (1, 0, 2, 1, 0));
      ...
      --ile potrzeba danych produktow na zestaw
